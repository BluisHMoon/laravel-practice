...
function themify_icons_edit_nav_menu_walker( $walker ) {
	...
	return 'Themify_Icons_Walker_Nav_Menu_Edit';
}
add_filter( 'wp_edit_nav_menu_walker', 'themify_icons_edit_nav_menu_walker' );
...
?>
	<p class="field-themify-icon description description-thin">
		<label for="edit-menu-item-ti_icon-<?php echo $item_id; ?>">
			...
			<a class="button button-secondary hide-if-no-js themify-choose-icon-trigger" href="#" data-target="#edit-menu-item-ti_icon-<?php echo $item_id ?>"><?php _e( 'Insert Icon', 'themify-icons' ); ?></a>
		</label>
	</p>
<?php }
...
function themify_icons_the_title( $title, $id ) {
	if( $icon = themify_icons_get_menu_icon( $id ) ) {
		...
		$title = '<i class="themify-menu-icon ' . $icon . '"></i> ' . $title;
	}
