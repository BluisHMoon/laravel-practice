...
function themify_icons_in_widget_form( $widget, $return, $instance ) {
	...
	<p>
		...
		<a class="button button-secondary hide-if-no-js themify-choose-icon-trigger" href="#" data-target="#<?php echo $widget->get_field_id( 'ti_icon' ); ?>"><?php _e( 'Insert Icon', 'themify-icons' ); ?></a>
	</p><?php
}
...
function themify_icons_dynamic_sidebar_params( $params ) {
	...
	if( isset( $options[$widget_id]['ti_icon'] ) && '' != $options[$widget_id]['ti_icon'] ) {
		...
		$params[0]['before_title'] .= '<i class="themify-menu-icon ' . $options[$widget_id]['ti_icon'] . '"></i> ';
	}
